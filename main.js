/**
 * SISTEMA OPERATIVO S.I.L.V.A. - VERSIÓN CORREGIDA
 * Dashboard PAR + LMSGI + Banco de 20 Preguntas
 */
...
async function cargarSistema() {
    ...
    try {
        ...
        console.error("Error en la matriz de datos.");
    }
}
...
function renderizarPortada(datos) {
    ...
    html += `
        <article class="card" style="border: 2px dashed #ff0055;">
            <div class="badge" style="background:#ff0055">WAR_GAME</div>
            <h2>TEST FINAL</h2>
...
function navegarA(siglas) {
    ...
    if (unidades.length > 0) {
        contenedor.innerHTML = `
            ...
            <h2 style="color:#00ffff">${siglas} > DASHBOARD</h2>
...
function iniciarTest() {
    ...
    if (r && r.toLowerCase() === p.a.toLowerCase()) alert("SUCCESS: Respuesta correcta.");
    else alert(`FAILURE: La solución era: ${p.a}`);
}
